...
const supportedVariableTypes = ['adhoc', 'query', 'interval'];
export class DataSource extends DataSourceApi<GrafanaQuery, GenericOptions> {
  ...
  query(options: QueryRequest): Promise<DataQueryResponse> {
    ...
    return lastValueFrom(
      this.doFetch<any[]>({
        url: `${this.url}/query?responseFormat=table`,
        data: request,
        method: 'POST',
      }).pipe(
        map((response) => {
  ...
  metricFindQuery(variableQuery: VariableQuery, options?: any, type?: string): Promise<MetricFindValue[]> {
    const test = options;
    const allMetricsApi = `${this.url}/metrics?resourceType=ALL&flatFormat=true`;
    const recType: string = test === undefined ? `ALL` : test.value;
    var filteredMetricsApi = `${this.url}/metrics?resourceType=${recType}&flatFormat=true`;
    const dashUrl = recType === `ALL` ? allMetricsApi : filteredMetricsApi;
    var editorUrl = `${this.url}/search`;
    const url = dashUrl.includes('undefined') ? editorUrl : dashUrl;
    const interpolated =
  ...
  resourceTypeFindQuery(variableQuery: VariableQuery, options?: any, type?: string): Promise<MetricFindValue[]> {
    console.log(options);
    const resourceTypeApi = `${this.url}/resourceTypes?flatFormat=true&appendAll=true`;
    const url = resourceTypeApi;
    const interpolated =
      variableQuery.format === 'json'
        ? JSON.parse(getTemplateSrv().replace(variableQuery.query, undefined, 'json'))
        : {
